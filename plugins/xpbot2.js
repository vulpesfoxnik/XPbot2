...
XPBot2.prototype.banUser = adminOnly(function banUser(args, data) {
    ...
    const parsedArgs = argRegex.exec(args);
    if (!parsedArgs) {
    ...
    const userId = parsedArgs[1] || parsedArgs[2];
    return db.models.User.banById(userId).then(user => {
        reply(`Master, ${user.userCode} has been banished from the realm!`);
        return user;
    }, (reject) => {
        reply(`Master, I have failed. ${userId} could not be banished!`);
        return reject;
    }).catch(err => {
        reply(`Master, I was unable to find ${userId},`)
    });
});
